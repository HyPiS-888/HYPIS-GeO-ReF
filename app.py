"""
HyPIS Ug  v7.9  — FAO-56 + XGBoost ML Integration
═══════════════════════════════════════════════════════════════════════════════
FIXES in v7.9 (over v7.8):
  ✔ CRITICAL — ML model feature mismatch fixed: model was trained with 8
    features including soil_pwp, but code only passed 7 (omitting soil_pwp).
    This caused a ValueError silently swallowed by except→return None, so
    every ML prediction showed "None". Fix: added soil_pwp to ML_FEATURES
    and to ml_predict_iwr() signature; updated all three call sites to pass
    soil["pwp"] / ACTIVE_PWP.
  ✔ CRITICAL — ml_predict_iwr() exception now surfaces the error message
    into ML_STATUS instead of silently returning None, making future
    prediction errors visible in the sidebar debug panel.
  ✔ Model .pkl re-saved with current XGBoost version to avoid pickle/version
    deserialization errors on deployment.
...
Author: Prosper BYARUHANGA · HyPIS App v7.9 · FAO-56 PM + XGBoost ML
═══════════════════════════════════════════════════════════════════════════════
"""
...
ML_OK       = False
ML_STATUS   = ""
# v7.9 FIX: 8 features — model was trained with soil_pwp; omitting it caused ValueError
# and silent None for all ML predictions.
...
def ml_predict_iwr(tmean, rh, wind, kc, precipitation, soil_fc, soil_pwp, root_depth, taw=None):
    """
    ...
    Features (in order, matching the trained model — 8 features):
      ...
      soil_pwp     - permanent wilting point (volumetric, 0-1)  v7.9 FIX
      root_depth   - rooting depth (m)

    v7.9 FIX: Added soil_pwp (8th feature). Model was trained with soil_pwp but
    the code was only passing 7 features, causing a ValueError that was silently
    swallowed, resulting in None for every ML prediction.
    """
    ...
    try:
        ...
        X = pd.DataFrame([[
            ...
            float(precipitation), float(soil_fc), float(soil_pwp), float(root_depth)
        ]], columns=ML_FEATURES)
        pred = float(ML_MODEL.predict(X)[0])
        upper = float(taw) if taw and taw > 0 else 50.0
        return round(max(0.0, min(upper, pred)), 3)
    except Exception as e:
        # v7.9: surface the real error so it is visible during debugging
        global ML_STATUS
        ML_STATUS = f"Prediction error: {e}"
        return None
...
<span class="hx-sub">HydroPredict · IrrigSched · Uganda Multi-Location IWR v7.9</span>
</div></div>
...
def run_water_balance(daily_df, crop, soil, planting_ts, sm_pct,
    ...
    for _, row in df.iterrows():
        ...
        _ml_nir = ml_predict_iwr(_tmean, _rh, _wind, row["kc"],
                                  _prec, soil["fc"], soil["pwp"], zr, taw=taw)
...
with tab1:
    ...
    st.caption(
        ...
        f"FAO-56 PM + XGBoost ML v7.9 · Trigger: Dr > RAW (soil depleted past MAD) · "
    ...
    if st.button("🧮 Calculate Today's IWR + Past 5 Days Water History",
        ...
        _ml_today = ml_predict_iwr(
            ...
            prec_in, ACTIVE_FC, ACTIVE_PWP, cp1["zr"], taw=taw1)
            ...
            if "ML_NIR" not in hist_r.columns or hist_r["ML_NIR"].isna().all():
                hist_r["ML_NIR"] = [
                    ml_predict_iwr(
                        ...
                        r.get("precipitation",0.), ACTIVE_FC, ACTIVE_PWP, cp3["zr"], taw=taw3)
...
st.markdown("---")
st.caption(
    f"HyPIS Ug v7.9 · {SITE_NAME} ({LAT}°, {LON}°, {ELEV} m) · "
