...
def soil_status_label(dr, raw, taw, pe, etc):
    ...
    CORRECT RULE (FAO-56):
    ─────────────────────────────────────────────────────────────────────────
    Irrigation is triggered when root-zone depletion Dr > RAW (Readily
    Available Water = MAD × TAW).  This is independent of today's rain.

    Rain DOES reduce the NIR for today, but it does NOT override the Dr > RAW
    trigger — if the soil is already stressed (Dr > RAW) a small rain event
    does not restore field capacity and the farmer must still irrigate.

    Rain-covered status only applies when Dr is BELOW the MAD threshold
    (soil still has adequate moisture) — i.e. the rain arrived before stress.
    ─────────────────────────────────────────────────────────────────────────
    """
    if dr <= 0:
        return "🟢 At Field Capacity", False
    if dr <= raw * 0.5:
        # Well within safe zone — rain or no rain, no irrigation needed
        if pe >= etc:
            return "🌧️ Rain covered — soil adequate", False
        return "✅ Adequate moisture", False
    if dr <= raw:
        # Approaching MAD threshold but not yet stressed
        if pe >= etc:
            return "🌧️ Rain topped up — monitor tomorrow", False
        return "🟡 Monitor — nearing MAD threshold", False
    # ── Dr > RAW: soil is BELOW the MAD threshold → irrigation required ──
    # Rain today may reduce NIR but cannot cancel the stress already present
    if dr <= taw:
...
with tab2:
    ...
    if st.button("📥 Get 5-Day Forecast + Volume", type="primary",
        ...
        if daily is None or daily.empty:
            ...
            # Per-column rounding — keep display clean (2 dp for mm, 1 dp for temps)
            _round2 = ["Pe mm","ET₀-PM mm/d","ETc mm/d","Dr Deficit mm","NIR mm","IWR gross mm"]
            _round1 = ["Tmax °C","Tmin °C","RH %","Rain mm","Vol m³","Pump min"]
            _round3 = ["Kc"]
            for c in _round2:
                if c in tb2: tb2[c] = tb2[c].round(2)
            for c in _round1:
                if c in tb2: tb2[c] = tb2[c].round(1)
            for c in _round3:
                if c in tb2: tb2[c] = tb2[c].round(3)
...
with tab3:
    ...
    if st.button("📥 Retrieve Historical Data", type="primary",
        ...
        if hist is None or hist.empty:
            ...
            # Per-column rounding
            _r2h = ["Pe mm","ET₀-PM mm/d","ET₀-H mm/d","ETc mm/d",
                    "Dr Deficit mm","NIR mm","IWR gross mm"]
            _r1h = ["Tmax °C","Tmin °C","RH %","Rain mm","Vol m³","Pump min"]
            for c in _r2h:
                if c in ht: ht[c] = ht[c].round(2)
            for c in _r1h:
                if c in ht: ht[c] = ht[c].round(1)
            ht["Kc"] = ht["Kc"].round(3)
